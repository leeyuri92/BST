...
function TK_funCheck(k)
{
    ...
    try {
			...
			{
	    		var tk_func = new Function('return' + TK_funArrary[i])();
    ...
    try {
		...
		for (var i = 0; i < document.forms.length; i++) {
				...
				if(TK_IsNal(document.forms[i].elements[j].name)){
					...
					if(isSecurity(document.forms[i].elements[j]) && 
...
function TK_KeyDown(nsEvent) {
    ...
    try {
       ...
       if(tekOption.senseReader == "true"){
		if ((keycode == 0x6A || keycode == 106) && seReaderInputSecurity(inputObj) == true) {
			return TK_InputCancle(nsEvent);
		}
		}
...
function TK_Input(nsEvent) {
	...
	*/
    ...
    if (tekOption.keyboardonly == "true" && bSecurity==true)
    {
		...
		if(isSecurity(TouchEnKeyNXE_CurObj)){ 
...
// 20230426
function TK_RealStopProcess(activeObj, TK_isSecurity)
{
...
function update_callback(result) {
    ...
    if (result.GetActiveElement != undefined) {
        ...
        if (activeObj != undefined) {
            ...
            if (TK_RealStopProcess(activeObj, TK_isSecurity) == false) return;
        }
    }
...
function Create_E2E_element_Form(eleObj) {
    ...
    if (!TK_IsNal(eleObj)) {
		...
		if(TK_IsNal(eleObj.name) && isSecurity(eleObj)
...
// 20230426
// 비보안필드에서 isDefaultSecurityOn true가 발생하여 senseReader 보안필드 조건사용을 위해 isDefaultSecurityOn를 제외한 seReaderInputSecurity 생성
function seReaderInputSecurity(inputObj) {
	if (isEnc(inputObj))
		return true;
